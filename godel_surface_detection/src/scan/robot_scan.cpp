...
static bool loadPoseParam(ros::NodeHandle& nh, const std::string& name,
...
bool RobotScan::init() {
	move_group_ptr_ = MoveGroupPtr(
			new move_group_interface::MoveGroup(params_.group_name));
...
bool RobotScan::load_parameters(const std::string& filename) {
	...
	return loadParam(nh, "group_name", params_.group_name)
			...
			&& loadParam(nh, "reachable_scan_points_ratio",
					params_.reachable_scan_points_ratio)
			&& loadParam(nh, "scan_target_frame", params_.scan_target_frame)
			&& loadBoolParam(nh, "stop_on_planning_error",
					params_.stop_on_planning_error);
}

void RobotScan::save_parameters(const std::string& filename) {
	if (!godel_param_helpers::toFile(filename, params_)) {
		ROS_WARN_STREAM(
				"Unable to save macro scan-parameters to: " << filename);
	}
}
...
bool RobotScan::generate_scan_display_trajectory(
		moveit_msgs::DisplayTrajectory& traj_data) {
	// create trajectory
...
void RobotScan::get_latest_scan_poses(geometry_msgs::PoseArray& poses) {
	poses.poses.insert(poses.poses.begin(), scan_traj_poses_.begin(),
...
void RobotScan::publish_scan_poses(std::string topic) {
	ros::NodeHandle nh;
	ros::Publisher poses_pub = nh.advertise<geometry_msgs::PoseArray>(topic, 1,
			true);
...
int RobotScan::scan(bool move_only) {
	...
	if (create_scan_trajectory(scan_traj_poses_, robot_traj)) {
		...
		// inserting all poses
		trajectory_poses.push_back(
				move_group_ptr_->getCurrentPose(params_.tcp_frame).pose);
		trajectory_poses.insert(trajectory_poses.begin() + 1,
				scan_traj_poses_.begin(), scan_traj_poses_.end());
		...
		for (size_t i = 1; i < trajectory_poses.size(); i++) {
			...
			move_group_ptr_->setPoseTarget(trajectory_poses[i],
					params_.tcp_frame);
			...
			if (!success) {
				if (params_.stop_on_planning_error) {
					ROS_ERROR_STREAM(
							"Path Planning to scan position " << i << " failed, quitting scan");
					break;
				} else {
					ROS_WARN_STREAM(
							"Path Planning to scan position " << i << " failed, skipping scan");
					continue;
				}
			}
			...
			if (move_group_ptr_->execute(my_plan)) {
				...
				if (params_.stop_on_planning_error) {
					ROS_ERROR_STREAM(
							"Path Execution to scan position " << i << " failed, quitting scan");
					break;
				} else {
					ROS_WARN_STREAM(
							"Path Execution to scan position " << i << " failed, skipping scan");
					continue;
				}
			}
			...
			if (!move_only) {
				...
				sensor_msgs::PointCloud2ConstPtr msg =
						ros::topic::waitForMessage<sensor_msgs::PointCloud2>(
								params_.scan_topic,
								ros::Duration(WAIT_MSG_DURATION));
				...
				if (msg) {
					ROS_INFO_STREAM(
							"Cloud message received, converting to target frame '" << params_.scan_target_frame << "'");
					...
					// transforming
					if (msg->header.frame_id.compare(params_.scan_target_frame)
							!= 0) {
						try {
							if (i == 1) {
								tf_listener_ptr_->lookupTransform(
										params_.scan_target_frame,
										msg->header.frame_id, ros::Time(0),
										*source_to_target_tf_permanent);
							}
							pcl_ros::transformPointCloud(*cloud_ptr, *cloud_ptr,
									*source_to_target_tf_permanent);
...
bool RobotScan::create_scan_trajectory(
	...
	geometry_msgs::Pose pose;
	double alpha;
	double alpha_incr = (params_.sweep_angle_end - params_.sweep_angle_start)
			/ (params_.num_scan_points - 1);
	double eef_step = 4 * alpha_incr * params_.cam_to_obj_xoffset;
	double jump_threshold = 0.0f;

	// relative transforms
	tf::Transform xoffset_disp = tf::Transform(tf::Quaternion::getIdentity(),
			tf::Vector3(params_.cam_to_obj_xoffset, 0, 0));
	tf::Transform zoffset_disp = tf::Transform(tf::Quaternion::getIdentity(),
			tf::Vector3(0, 0, params_.cam_to_obj_zoffset));
	tf::Transform rot_alpha_about_z = tf::Transform::getIdentity();
	tf::Transform rot_tilt_about_y = tf::Transform(
			tf::Quaternion(tf::Vector3(0, 1, 0), params_.cam_tilt_angle));
	for (int i = 0; i < params_.num_scan_points; i++) {
		alpha = params_.sweep_angle_start + alpha_incr * i;
		rot_alpha_about_z = tf::Transform(
				tf::Quaternion(tf::Vector3(0, 0, 1), alpha));
		obj_to_cam_pose = zoffset_disp * rot_alpha_about_z * xoffset_disp
				* rot_tilt_about_y;
		world_to_tcp = world_to_obj_tf * obj_to_cam_pose
				* tcp_to_cam_tf.inverse();
		tf::poseTFToMsg(world_to_tcp, pose);
		scan_poses.push_back(pose);
	}

	ROS_INFO_STREAM(
			"Computing cartesian path for a trajectory with " << params_.num_scan_points << " points, eef_step: " << eef_step);
	move_group_ptr_->setEndEffectorLink(params_.tcp_frame);

	ROS_INFO_STREAM(
			"Computing cartesian path for link '" << move_group_ptr_->getEndEffectorLink() << "'");
...
void RobotScan::apply_trajectory_parabolic_time_parameterization(
		...
		moveit_msgs::RobotTrajectory& traj, unsigned int max_iterations,
		double max_time_change_per_it) {

	// applying filter
	trajectory_processing::IterativeParabolicTimeParameterization iter_prmt(
	...
	std::vector<trajectory_msgs::JointTrajectoryPoint>::iterator iter =
			points.begin() + 1;
	double dt;
